            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                <div class="space-y-6">
                    ...
                    <p class="text-lg text-gray-700 leading-relaxed">
                        আপনার প্রজেক্টকে অনন্য রূপ দিন আমাদের প্রিমিয়াম {{ strtolower($productTitle ?? 'বই') }} প্রিন্টিং
                ...
                <div class="relative">
                    <div class="aspect-[4/3] rounded-2xl overflow-hidden shadow-2xl">
                        <img src="{{ $productImage ?? 'https://placehold.co/800x600?text=' . urlencode($productTitle ?? 'Book') }}"
                            alt="{{ $productTitle ?? 'Book' }}" class="w-full h-full object-cover">
                    </div>
                    <!-- Decorative elements -->
    ...
    <section class="bg-white py-16" id="configure">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form id="addToCartForm" action="{{ route('cart.add') }}" method="POST">
                ...
                <input type="hidden" name="title" value="{{ $productTitle ?? 'Book' }}">
                <input type="hidden" name="category" value="{{ $category ?? 'books' }}">
                <input type="hidden" name="product_type" value="{{ $productType ?? 'book' }}">
                    ...
                    <div class="lg:col-span-2 space-y-8">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                @forelse($configOptions['bindings'] ?? [] as $binding)
                                    <button type="button" data-category="binding"
                                        data-price="{{ is_array($binding) ? ($binding['price'] ?? 0) : 0 }}"
                                        ...
                                        <div
                                            ...
                                            <svg class="w-24 h-24 text-gray-400" viewBox="0 0 100 100" fill="none"
                                                ...
                                                @php $bindingLabel = is_array($binding) ? ($binding['label'] ?? '') : (is_string($binding) ? $binding : ''); @endphp
                                                @if(stripos($bindingLabel, 'saddle') !== false)
                                                    <path d="M30 50 L45 65 Q55 60 65 50 L65 30" />
                                                    <line x1="35" y1="40" x2="35" y2="70" />
                                                    <line x1="40" y1="35" x2="40" y2="75" />
                                                @elseif(stripos($bindingLabel, 'perfect') !== false)
                                        ...
                                        <div class="text-base font-semibold text-blue-600">{{ $bindingLabel }}</div>
                                        @if(is_array($binding) && ($binding['price'] ?? 0) > 0)
                                            <div class="text-sm text-gray-500 mt-1">+${{ number_format($binding['price'], 2) }}
                                            </div>
                                        @endif
                                    </button>
                                @empty
                        ...
                        <div>
                            ...
                            <div class="flex gap-4 overflow-x-auto pb-2">
                                @forelse($configOptions['sizes'] ?? [] as $size)
                                    <button data-category="size" data-price="{{ is_array($size) ? ($size['price'] ?? 0) : 0 }}"
                                        ...
                                        <div
                                            ...
                                            <div class="text-gray-600 text-xs font-medium">
                                                {{ is_array($size) ? ($size['label'] ?? '') : $size }}</div>
                                        </div>
                                        <div class="text-sm font-semibold text-gray-700">
                                            {{ is_array($size) ? ($size['label'] ?? '') : $size }}</div>
                                        @if(is_array($size) && ($size['price'] ?? 0) > 0)
                                            <div class="text-xs text-blue-600 mt-1">+${{ number_format($size['price'], 2) }}</div>
                                        @endif
                                    </button>
                                @empty
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                @forelse($configOptions['orientations'] ?? [] as $orientation)
                                    <button data-category="orientation"
                                        data-price="{{ is_array($orientation) ? ($orientation['price'] ?? 0) : 0 }}"
                                        ...
                                        <div class="text-lg font-bold text-gray-700">
                                            {{ is_array($orientation) ? ($orientation['label'] ?? '') : $orientation }}</div>
                                        @if(is_array($orientation) && ($orientation['price'] ?? 0) > 0)
                                            <div class="text-sm text-blue-600">+${{ number_format($orientation['price'], 2) }}</div>
                                        @endif
                                    </button>
                                @empty
                        ...
                        <div>
                            ...
                            <div class="border border-gray-300 rounded-lg overflow-hidden">
                                <div class="p-6 space-y-4">
                                    @forelse($configOptions['paperTypes'] ?? $configOptions['paper_types'] ?? [] as $paper)
                                        <label class="flex items-center gap-3 cursor-pointer group">
                                            <input type="radio" name="paper_type"
                                                value="{{ is_array($paper) ? ($paper['value'] ?? ($paper['label'] ?? '')) : $paper }}"
                                                data-category="paper"
                                                data-price="{{ is_array($paper) ? ($paper['price'] ?? 0) : 0 }}"
                                                class="w-5 h-5 text-blue-600 border-gray-300 focus:ring-blue-500">
                                            <div class="flex-1 flex items-center justify-between">
                                                <span
                                                    class="text-base font-semibold text-blue-600">{{ is_array($paper) ? ($paper['label'] ?? '') : $paper }}</span>
                                                @if(is_array($paper) && ($paper['price'] ?? 0) > 0)
                                                    <div class="text-sm font-medium text-gray-500">
                                                        +${{ number_format($paper['price'], 2) }}</div>
                                                @endif
                                            </div>
                                        </label>
                                    @empty
                        ...
                        <!-- Cover Paper Type -->
                        @if(!empty($configOptions['coverPaperTypes']) || !empty($configOptions['cover_papers']))
                            <div>
                                <h3 class="text-2xl font-bold text-gray-700 mb-4">Cover Paper Type</h3>
                                <div class="border border-gray-300 rounded-lg overflow-hidden">
                                    <div class="p-6 space-y-4">
                                        @forelse($configOptions['coverPaperTypes'] ?? $configOptions['cover_papers'] ?? [] as $cover)
                                            <label class="flex items-center gap-3 cursor-pointer group">
                                                <input type="radio" name="cover_paper_type"
                                                    value="{{ is_array($cover) ? ($cover['value'] ?? ($cover['label'] ?? '')) : $cover }}"
                                                    data-category="cover"
                                                    data-price="{{ is_array($cover) ? ($cover['price'] ?? 0) : 0 }}"
                                                    class="w-5 h-5 text-blue-600 border-gray-300 focus:ring-blue-500">
                                                <div class="flex-1 flex items-center justify-between">
                                                    <span
                                                        class="text-base font-semibold text-blue-600">{{ is_array($cover) ? ($cover['label'] ?? '') : $cover }}</span>
                                                    @if(is_array($cover) && ($cover['price'] ?? 0) > 0)
                                                        <div class="text-sm font-medium text-gray-500">
                                                            +${{ number_format($cover['price'], 2) }}</div>
                                                    @endif
                                                </div>
                                            </label>
                                        @empty
                        ...
                        <!-- Coating -->
                        @if(!empty($configOptions['coatings']))
                            <div>
                                <div class="flex items-center gap-2 mb-4">
                                ...
                                <div class="relative">
                                    <select id="coatingSelect" data-category="coating"
                                        ...
                                        @foreach($configOptions['coatings'] as $coating)
                                            <option
                                                value="{{ is_array($coating) ? ($coating['value'] ?? ($coating['label'] ?? '')) : $coating }}"
                                                data-price="{{ is_array($coating) ? ($coating['price'] ?? 0) : 0 }}">
                                                {{ is_array($coating) ? ($coating['label'] ?? '') : $coating }}
                                                @if(is_array($coating) && ($coating['price'] ?? 0) > 0)(+${{ number_format($coating['price'], 2) }})@endif
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <!-- Finishes -->
                        @if(!empty($configOptions['finishes']))
                            <div>
                                <h3 class="text-2xl font-bold text-gray-700 mb-4">Finishes</h3>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    @foreach($configOptions['finishes'] as $finish)
                                        <label
                                            ...
                                            <input type="checkbox" name="finish[]"
                                                value="{{ $finish['value'] ?? $finish['label'] }}" data-category="finish"
                                                data-price="{{ $finish['price'] ?? 0 }}"
                                                class="w-5 h-5 text-blue-600 border-gray-300 focus:ring-blue-500 rounded">
                                            <div class="flex-1 flex items-center justify-between">
                                                <span
                                                    class="text-base font-medium text-gray-700">{{ is_array($finish) ? ($finish['label'] ?? '') : $finish }}</span>
                                                @if(is_array($finish) && ($finish['price'] ?? 0) > 0)
                                                    <div class="text-sm font-medium text-blue-600">
                                                        +${{ number_format($finish['price'], 2) }}</div>
                                                @endif
                                            </div>
                                        </label>
                                    @endforeach
