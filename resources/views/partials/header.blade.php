    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16 md:h-20">
            ...
            <div class="flex items-center flex-shrink-0">
                <a href="/" class="flex items-center">
                    @if(!empty($headerContent['logo']))
                        <img src="{{ $headerContent['logo'] }}" alt="{{ $headerContent['site_name'] ?? config('site.name') }}" class="h-10 md:h-12 w-auto">
                    @else
                        <img src="{{ asset('logo.png') }}" alt="{{ $headerContent['site_name'] ?? config('site.name') }}" class="h-10 md:h-12 w-auto">
                    @endif
                </a>
            </div>
            ...
            <div class="flex items-center gap-2 sm:gap-4">
                <a href="tel:{{ $headerContent['phone'] ?? config('site.phone') }}" class="hidden lg:flex items-center gap-2 text-gray-700 hover:text-gray-900 text-sm">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                    </svg>
                    <span>{{ $headerContent['phone'] ?? config('site.phone') }}</span>
                </a>
                    ...
                    <div class="relative" x-data="{ open: false }" @click.away="open = false">
                        <button @click="open = !open" class="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-gray-100 transition">
                            @if(Auth::user()->profile_image)
                                <img src="{{ Auth::user()->profile_image }}" alt="{{ Auth::user()->name }}" class="w-8 h-8 rounded-full object-cover">
                            @else
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="overflow-x-auto scrollbar-hide -mx-4 sm:-mx-6 lg:-mx-8">
                <div class="flex items-center gap-4 sm:gap-6 md:gap-8 h-12 md:h-14 pl-4 sm:pl-6 lg:pl-8 pr-4 sm:pr-6 lg:pr-8">
                    @if(isset($headerContent['navigation']) && count($headerContent['navigation']) > 0)
                        @foreach($headerContent['navigation'] as $navItem)
                            <a href="{{ $navItem['url'] }}" class="{{ request()->is(ltrim($navItem['pattern'] ?? $navItem['url'], '/')) ? 'text-blue-600 font-semibold' : 'text-gray-700 hover:text-gray-900' }} text-xs sm:text-sm md:text-base whitespace-nowrap">{{ $navItem['title'] }}</a>
                        @endforeach
                    @else
